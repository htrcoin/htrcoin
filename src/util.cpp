...
int nAnonymizeHighTemperatureAmount = 1000;
...
#else
    const char* pszModule = "HighTemperature";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HighTemperature
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HighTemperature
    // Mac: ~/Library/Application Support/HighTemperature
    // Unix: ~/.HighTemperature
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HighTemperature";
#else
    fs::path pathRet;
    ...
    return pathRet / "HighTemperature";
#else
    // Unix
    return pathRet / ".HighTemperature";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "HighTemperature.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "HighTemperatured.pid"));
