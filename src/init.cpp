...
void Shutdown()
{
    ...
    RenameThread("HighTemperature-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: HighTemperature.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: HighTemperatured.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizehightemperatureamount=<n> " + _("Keep N HighTemperature anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. HighTemperature is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. HighTemperature is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("HighTemperature version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "HighTemperature server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of HighTemperature") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart HighTemperature to complete") << "\n";
    ...
    nAnonymizeHighTemperatureAmount = GetArg("-anonymizehightemperatureamount", 0);
    if(nAnonymizeHighTemperatureAmount > 999999) nAnonymizeHighTemperatureAmount = 999999;
    if(nAnonymizeHighTemperatureAmount < 2) nAnonymizeHighTemperatureAmount = 2;
    ...
    LogPrintf("Anonymize HighTemperature Amount %d\n", nAnonymizeHighTemperatureAmount);
