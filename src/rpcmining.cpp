...
Value getstakinginfo(const Array& params, bool fHelp)
{
	...
	unsigned int nTempSpacing = GetTargetSpacing(nBestHeight);
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "HighTemperature is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "HighTemperature is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "HighTemperature is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "HighTemperature is downloading blocks...");
