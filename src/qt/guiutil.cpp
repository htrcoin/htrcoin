...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("HighTemperature"))
        return false;
...
{
    // Convert HighTemperature:// to HighTemperature:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("HighTemperature://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "HighTemperature:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "HighTemperature.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for HighTemperature.lnk
...
{
    return GetAutostartDir() / "HighTemperature.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a HighTemperature.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=HighTemperature\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the HighTemperature app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add HighTemperature app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("HighTemperature-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  HighTemperature-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("HighTemperature-Qt"));
